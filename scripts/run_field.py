"""Process every field photo: ArUco calibration → SAM3 leaf detection → main-plant filter."""
...
from beanleafmapper.pipeline.field import process_field_directory
...
def main() -> None:
    ...
    parser.add_argument("--max-distance-cm", type=float, default=None)
    ...
    if args.max_distance_cm is not None:
        cfg.detection.main_plant_max_distance_cm = args.max_distance_cm
    ...
    df = process_field_directory(cfg)
    print(f"Processed {df['image'].nunique() if not df.empty else 0} field images.")
    print(f"Wrote outputs to {cfg.output_dir / 'field'}")
